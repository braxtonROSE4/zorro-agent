...
This directory contains the integration layer between **zorro-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  ZorroAgentBaseEnv    │  zorro_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     ZorroSweEnv    TerminalBench2EvalEnv
...
**ZorroAgentBaseEnv** (`zorro_base_env.py`) extends BaseEnv with zorro-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves zorro-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `ZorroAgentBaseEnv` and implement:
...
`ZorroAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as zorro-agent's `run_agent.py`:
...
- **Generic**: `call_tool(name, args)` -- call any zorro-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some zorro-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `zorro` -- Zorro/ChatML `<tool_call>` XML format
...
├── zorro_base_env.py            # Abstract base (ZorroAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (ZorroAgentLoop)
...
│
├── zorro_swe_env/               # SWE-bench style training environment
│   └── zorro_swe_env.py
│
...
### ZorroSweEnv (`zorro_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/zorro_swe_env/zorro_swe_env.py serve \
...
2. Create your env file inheriting from `ZorroAgentBaseEnv`
3. Implement the four abstract methods + `evaluate()`

```python
from environments.zorro_base_env import ZorroAgentBaseEnv, ZorroAgentEnvConfig

class MyEnvConfig(ZorroAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(ZorroAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `ZorroAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which zorro toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `zorro` |
