...
# Resolve Zorro home directory (respects ZORRO_HOME override)
from zorro_constants import get_zorro_home
from utils import atomic_yaml_write, is_truthy_value
_zorro_home = get_zorro_home()

# Load environment variables from ~/.zorro/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from zorro_cli.env_loader import load_zorro_dotenv
_env_path = _zorro_home / '.env'
load_zorro_dotenv(zorro_home=_zorro_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _zorro_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["ZORRO_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            # Bridge agent.gateway_timeout → ZORRO_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "ZORRO_AGENT_TIMEOUT" not in os.environ:
                os.environ["ZORRO_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg and "ZORRO_AGENT_TIMEOUT_WARNING" not in os.environ:
                os.environ["ZORRO_AGENT_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "gateway_notify_interval" in _agent_cfg and "ZORRO_AGENT_NOTIFY_INTERVAL" not in os.environ:
                os.environ["ZORRO_AGENT_NOTIFY_INTERVAL"] = str(_agent_cfg["gateway_notify_interval"])
            if "restart_drain_timeout" in _agent_cfg and "ZORRO_RESTART_DRAIN_TIMEOUT" not in os.environ:
                os.environ["ZORRO_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg and "ZORRO_GATEWAY_BUSY_INPUT_MODE" not in os.environ:
                os.environ["ZORRO_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
        # Timezone: bridge config.yaml → ZORRO_TIMEZONE env var.
        # ZORRO_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "ZORRO_TIMEZONE" not in os.environ:
            os.environ["ZORRO_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["ZORRO_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Apply IPv4 preference if configured (before any HTTP clients are created).
try:
    from zorro_constants import apply_ipv4_preference
...
os.environ["ZORRO_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["ZORRO_EXEC_ASK"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _zorro_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("ZORRO_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `zorro skills config`"
                    )

        # Check optional skills (shipped with repo but not installed)
        from zorro_constants import get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `zorro skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.zorro/config.yaml, returning {} on any error."""
    try:
        config_path = _zorro_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _zorro_home / 'config.yaml')
    return {}
...
def _resolve_zorro_bin() -> Optional[list[str]]:
    """Resolve the Zorro update command as argv parts.

    Tries in order:
    1. ``shutil.which("zorro")`` — standard PATH lookup
    2. ``sys.executable -m zorro_cli.main`` — fallback when Zorro is running
       from a venv/module invocation and the ``zorro`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("zorro_cli") is not None:
            return [sys.executable, "-m", "zorro_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the zorro-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("zorro-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _zorro_home / "gateway_voice_mode.json"
    ...
    def _resolve_session_agent_runtime(
        ...
        *,
        ...
        # (e.g. user ran `zorro auth add openai-codex` without `zorro model`),
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks ZORRO_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.zorro/config.yaml.
        Relative paths are resolved from ~/.zorro/.
        """
        import json as _json
        file_path = os.getenv("ZORRO_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _zorro_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks ZORRO_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.zorro/config.yaml.
        """
        prompt = os.getenv("ZORRO_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_busy_input_mode() -> str:
        ...
        mode = os.getenv("ZORRO_GATEWAY_BUSY_INPUT_MODE", "").strip().lower()
        if not mode:
            try:
                import yaml as _y
                cfg_path = _zorro_home / "config.yaml"
    ...
    def _load_restart_drain_timeout() -> float:
        ...
        raw = os.getenv("ZORRO_RESTART_DRAIN_TIMEOUT", "").strip()
        if not raw:
            try:
                import yaml as _y
                cfg_path = _zorro_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("ZORRO_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _zorro_home / "config.yaml"
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        zorro_cmd = _resolve_zorro_bin()
        if not zorro_cmd:
            logger.error("Could not locate zorro binary for detached /restart")
            return

        current_pid = os.getpid()
        cmd = " ".join(shlex.quote(part) for part in zorro_cmd)
        shell_cmd = (
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Zorro Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.zorro/.env to allow open access, "
        ...
        # This prevents unwanted auto-resets after `zorro update`,
        # `zorro gateway restart`, or `/restart`.
        _clean_marker = _zorro_home / ".clean_shutdown"
        ...
        if not notified and any(
            path.exists()
            for path in (
                _zorro_home / ".update_pending.json",
                _zorro_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def stop(
        ...
        *,
        ...
        async def _stop_impl() -> None:
            ...
            try:
                (_zorro_home / ".clean_shutdown").touch()
            except Exception:
                pass
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'zorro-agent[slack]'")
                return None
            ...
            if not check_teams_requirements():
                logger.warning("Teams: botbuilder-core not installed or TEAMS_APP_ID/PASSWORD not set")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`zorro pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _zorro_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(os.getenv("ZORRO_AGENT_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from zorro_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in zorro_cli/commands.py — no hardcoded set to maintain here.
        from zorro_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                ...
                # hyphens. See zorro_cli/commands.py:_build_telegram_menu.
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Zorro venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill zorro-agent-setup`"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_data = {}
            try:
                _hyg_cfg_path = _zorro_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Zorro delivers cron job results "
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from ZORRO_HOME path
        # Profile paths look like: ~/.zorro/profiles/<name>
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            (_zorro_home / ".restart_notify.json").write_text(
        ...
        return "♻ Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `zorro gateway restart`."

    async def _handle_help_command(self, event: MessageEvent) -> str:
        """Handle /help command - list available commands."""
        from zorro_cli.commands import gateway_help_lines
        lines = [
            "📖 **Zorro Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        import yaml
        from zorro_cli.model_switch import (
        ...
        if persist_global:
            try:
                ...
                from zorro_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `zorro setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.zorro/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Zorro with the messaging extra, e.g. "
                    "`pip install zorro-agent[messaging]`."
                )
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _zorro_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            max_iterations = int(os.getenv("ZORRO_MAX_ITERATIONS", "90"))
    ...
    _UPDATE_ALLOWED_PLATFORMS = frozenset({
        ...
        Platform.FEISHU, Platform.WECOM, Platform.WECOM_CALLBACK, Platform.WEIXIN, Platform.BLUEBUBBLES, Platform.TEAMS, Platform.LOCAL,
    })

    async def _handle_debug_command(self, event: MessageEvent) -> str:
        """Handle /debug — upload debug report + logs and return paste URLs."""
        import asyncio
        from zorro_cli.debug import (
        ...
        def _collect_and_upload():
            ...
            lines.append("\nShare these links with the Zorro team for support.")
            return "\n".join(lines)

        return await loop.run_in_executor(None, _collect_and_upload)

    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Zorro Agent to the latest version.

        Spawns ``zorro update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``zorro update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from zorro_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `zorro update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Zorro Agent')}"
        ...
        if not zorro_cmd:
            return (
                "✗ Could not locate the `zorro` command. "
                "Zorro is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `zorro update` manually in your terminal."
            )

        pending_path = _zorro_home / ".update_pending.json"
        output_path = _zorro_home / ".update_output.txt"
        exit_code_path = _zorro_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `zorro update --gateway` detached so it survives gateway restart.
        ...
        zorro_cmd_str = " ".join(shlex.quote(part) for part in zorro_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {zorro_cmd_str} update --gateway"
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Zorro update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``zorro update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _zorro_home / ".update_pending.json"
        claimed_path = _zorro_home / ".update_pending.claimed.json"
        output_path = _zorro_home / ".update_output.txt"
        exit_code_path = _zorro_home / ".update_exit_code"
        prompt_path = _zorro_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Zorro update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Zorro update failed (exit code {}).".format(exit_code))
                ...
                (_zorro_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Zorro update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Zorro update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Zorro update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Zorro update finished successfully."
                    else:
                        msg = "❌ Zorro update failed. Check the gateway logs or run `zorro update` manually for details."
    ...
    async def _send_restart_notification(self) -> None:
        ...
        notify_path = _zorro_home / ".restart_notify.json"
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called zorro-agent-setup that can help "
                    "users configure Zorro features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called zorro-agent-setup "
                                "that can help users configure Zorro features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _resolved_tp
            or os.getenv("ZORRO_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["ZORRO_SESSION_KEY"] = session_key or ""
        ...
        # ZORRO_AGENT_NOTIFY_INTERVAL env var.  Default 600s (10 min).
        # 0 = disable notifications.
        _NOTIFY_INTERVAL_RAW = float(os.getenv("ZORRO_AGENT_NOTIFY_INTERVAL", 600))
        ...
        try:
            ...
            # ZORRO_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("ZORRO_AGENT_TIMEOUT", 1800))
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(os.getenv("ZORRO_AGENT_TIMEOUT_WARNING", 900))
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from zorro_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `zorro cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same ZORRO_HOME.
    # The PID file is scoped to ZORRO_HOME, so future multi-profile
    # setups (each profile using a distinct ZORRO_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            zorro_home = str(get_zorro_home())
            logger.error(
                "Another gateway instance is already running (PID %d, ZORRO_HOME=%s). "
                "Use 'zorro gateway restart' to replace it, or 'zorro gateway stop' first.",
                existing_pid, zorro_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'zorro gateway restart' to replace it,\n"
                f"   or 'zorro gateway stop' to kill it first.\n"
                f"   Or use 'zorro gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    setup_logging(zorro_home=_zorro_home, mode="gateway")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Zorro Gateway - Multi-platform messaging")
