...
Checklist for integrating a new messaging platform into the Zorro gateway.
...
And add it to the `zorro-gateway` composite:

```python
"zorro-gateway": {
    "includes": [..., "zorro-your-platform"]
}
```
...
## 12. Status Display (`zorro_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zorro_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zorro_cli/ toolsets.py \
  --include="*.py" -l | sort -u
