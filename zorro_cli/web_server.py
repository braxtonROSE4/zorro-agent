"""
Zorro Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m zorro_cli.main web          # Start on http://127.0.0.1:9119
    python -m zorro_cli.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'zorro web' to auto-install, or: pip install zorro-agent[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Zorro Agent", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Zorro supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
async def get_logs(
    ...
    from zorro_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zorro_home() / "logs" / log_name
...
async def get_skills():
    ...
    from zorro_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from zorro_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from zorro_cli.tools_config import (
...
def start_server(host: str = "127.0.0.1", port: int = 9119, open_browser: bool = True):
    ...
    print(f"  Zorro Web UI → http://{host}:{port}")
