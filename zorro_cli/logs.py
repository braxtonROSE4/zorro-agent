"""``zorro logs`` — view and filter Zorro log files.
...
Usage examples::
    ...
    zorro logs                    # last 50 lines of agent.log
    zorro logs -f                 # follow agent.log in real time
    zorro logs errors             # last 50 lines of errors.log
    zorro logs gateway -n 100    # last 100 lines of gateway.log
    zorro logs --level WARNING    # only WARNING+ lines
    zorro logs --session abc123   # filter by session ID substring
    zorro logs --component tools  # only tool-related lines
    zorro logs --since 1h         # lines from the last hour
    zorro logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zorro_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Zorro runs — try 'zorro chat' first)")
        sys.exit(1)
    ...
    if component:
        from zorro_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_zorro_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zorro_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zorro_home()}/logs/")
        return

    print(f"Log files in {display_zorro_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zorro chat' to generate logs)")
