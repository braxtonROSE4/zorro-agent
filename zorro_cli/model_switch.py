...
- ``zorro_cli.providers``        -- canonical provider identity + overlays
- ``zorro_cli.model_normalize``  -- per-provider name formatting
...
from zorro_cli.providers import (
...
_ZORRO_MODEL_WARNING = (
    "Brax Zorro 3 & 4 models are NOT agentic and are not designed "
    "for use with Zorro Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Brax Zorro 3 / Zorro 4 chat families.
# The previous substring check (`"zorro" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zorro-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zorro" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Zorro-3-Llama-3.1-70B, zorro-4-405b, openrouter/zorro3:70b
# Negative examples it must NOT match:
#   zorro-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ZORRO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zorro[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_zorro_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Zorro 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZORRO_NON_AGENTIC_RE.search(model_name))


def _check_zorro_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Zorro 3/4 chat model."""
    if is_nous_zorro_non_agentic(model_name):
        return _ZORRO_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    from zorro_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zorro model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zorro doctor' — config issues detected:"
    ...
    zorro_warn = _check_zorro_model_warning(new_model)
    if zorro_warn:
        warnings.append(zorro_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from zorro_cli/models.py (OPENROUTER_MODELS,
    ...
    from zorro_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by zorro provider ID
    ...
    # --- 1. Check Zorro-mapped providers ---
    for zorro_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(zorro_id)
        ...
        model_ids = curated.get(zorro_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Zorro-only providers (nous, openai-codex, copilot, opencode-go) ---
    from zorro_cli.providers import ZORRO_OVERLAYS
    from zorro_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Zorro provider ID.
    # ZORRO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Zorro IDs ("copilot").
    _mdev_to_zorro = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZORRO_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Zorro slug — e.g. "github-copilot" → "copilot"
        zorro_slug = _mdev_to_zorro.get(pid, pid)
        if zorro_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from zorro_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or zorro_slug in providers_store
                    or pid in pool_store or zorro_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(zorro_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zorro_slug, exc)
        ...
        # configured.
        if not has_creds and zorro_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_zorro_oauth_credentials,
                )
                zorro_creds = read_zorro_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (zorro_creds and zorro_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Zorro slug, fall back to overlay key
        model_ids = curated.get(zorro_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": zorro_slug,
            "name": get_label(zorro_slug),
            "is_current": zorro_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zorro",
