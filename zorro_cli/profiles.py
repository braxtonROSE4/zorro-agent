"""
Profile management for multiple isolated Zorro instances.

Each profile is a fully independent ZORRO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zorro/profiles/<name>/`` by default.

The "default" profile is ``~/.zorro`` itself — backward compatible,
zero migration needed.

Usage::

    zorro profile create coder          # fresh profile + bundled skills
    zorro profile create coder --clone  # also copy config, .env, SOUL.md
    zorro profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zorro -p coder chat                 # or via flag
    zorro profile use coder             # set as sticky default
    zorro profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zorro_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.zorro) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zorro-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "zorro", "default", "test", "tmp", "root", "sudo",
})

# Zorro subcommands that cannot be used as profile names/aliases
_ZORRO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zorro root, NOT to the current ZORRO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZORRO_HOME points outside
    ``~/.zorro``, profiles live under ``ZORRO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zorro_home() / "profiles"


def _get_default_zorro_home() -> Path:
    """Return the default (pre-profile) ZORRO_HOME path.

    In standard deployments this is ``~/.zorro``.
    In Docker/custom deployments where ZORRO_HOME is outside ``~/.zorro``
    (e.g. ``/opt/data``), returns ZORRO_HOME directly.
    """
    from zorro_constants import get_default_zorro_root
    return get_default_zorro_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_zorro_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.zorro
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZORRO_HOME directory."""
    if name == "default":
        return _get_default_zorro_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, zorro subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _ZORRO_SUBCOMMANDS:
        return f"'{name}' conflicts with a zorro subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zorro_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zorro)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZORRO_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZORRO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zorro).\n"
            "To remove everything, use: zorro uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZORRO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZORRO_HOME")
    try:
        os.environ["ZORRO_HOME"] = str(profile_dir)
        from zorro_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZORRO_HOME"] = old_home
        elif "ZORRO_HOME" in os.environ:
            del os.environ["ZORRO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zorro/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: zorro profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZORRO_HOME.

    Returns ``"default"`` if ZORRO_HOME is not set or points to ``~/.zorro``.
    Returns the profile name if ZORRO_HOME points into ``~/.zorro/profiles/<name>``.
    Returns ``"custom"`` if ZORRO_HOME is set to an unrecognized path.
    """
    from zorro_constants import get_zorro_home
    zorro_home = get_zorro_home()
    resolved = zorro_home.resolve()

    default_resolved = _get_default_zorro_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.zorro itself — its parent is ~/ and its
        # directory name is ".zorro", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zorro profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.zorro itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zorro). "
            "Specify a different name: zorro profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for zorro profile names."""
    return '''# Zorro Agent profile completion
# Add to ~/.bashrc: eval "$(zorro completion bash)"

_zorro_profiles() {
    local profiles_dir="$HOME/.zorro/profiles"
...
_zorro_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zorro_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zorro_completion zorro
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for zorro profile names."""
    return '''#compdef zorro
# Zorro Agent profile completion
# Add to ~/.zshrc: eval "$(zorro completion zsh)"

_zorro() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zorro/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.zorro/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZORRO_HOME path string.

    Called early in the CLI entry point, before any zorro modules
    are imported, to set the ZORRO_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: zorro profile create {profile_name}"
        )
