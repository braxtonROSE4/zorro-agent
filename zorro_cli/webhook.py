"""zorro webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    zorro webhook subscribe <name> [options]
    zorro webhook list
    zorro webhook remove <name>
    zorro webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.zorro/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _zorro_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: zorro gateway run
"""
...
def webhook_command(args):
    """Entry point for 'zorro webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: zorro webhook {subscribe|list|remove|test}")
        print("Run 'zorro webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (zorro gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: zorro webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from zorro webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (zorro gateway run)")
