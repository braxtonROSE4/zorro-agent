...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zorro -c.",
    ...
    "Ctrl+Z suspends Zorro to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zorro -c resumes your most recent CLI session. zorro -c \"project name\" resumes by title.",
    "zorro -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zorro -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zorro chat -t web,terminal enables only specific toolsets for a focused session.",
    "zorro chat -s github-pr-workflow preloads a skill at launch.",
    "zorro chat -q \"query\" runs a single non-interactive query and exits.",
    "zorro chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zorro chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zorro --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zorro chat --source telegram tags the session for filtering in zorro sessions list.",
    "zorro -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zorro doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zorro dump outputs a compact setup summary — great for bug reports.",
    "zorro config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zorro config edit opens config.yaml in your default editor.",
    "zorro config check scans for missing or stale configuration options.",
    "zorro sessions browse opens an interactive session picker with search.",
    "zorro sessions stats shows session counts by platform and database size.",
    "zorro sessions prune --older-than 30 cleans up old sessions.",
    "zorro skills search react --source skills-sh searches the skills.sh public directory.",
    "zorro skills check scans installed hub skills for upstream updates.",
    "zorro skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zorro skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zorro mcp add github --command npx adds MCP servers from the command line.",
    "zorro mcp serve runs Zorro itself as an MCP server for other agents.",
    "zorro auth add lets you add multiple API keys for credential pool rotation.",
    "zorro completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zorro logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zorro backup creates a zip backup of your entire Zorro home directory.",
    "zorro profile create coder creates an isolated profile that becomes its own command.",
    "zorro profile create work --clone copies your current config and keys to a new profile.",
    "zorro update syncs new bundled skills to ALL profiles automatically.",
    "zorro gateway install sets up Zorro as a system service (systemd/launchd).",
    "zorro memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zorro webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zorro -w).",
    ...
    "ZORRO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zorro tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zorro profile create coder' creates the 'coder' command.",
    "zorro profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zorro prints a resume command with session ID and stats.",
    "zorro sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zorro -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zorro skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zorro/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zorro cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zorro runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "zorro gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zorro via a local macOS server.",
    ...
    "Context files (.zorro.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.zorro/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Zorro loads project context from .zorro.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — zorro tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zorro/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zorro chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zorro plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zorro acp runs Zorro as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZORRO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zorro login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "zorro status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.zorro/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.zorro/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Zorro your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.zorro/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Zorro auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zorro/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ZORRO_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ZORRO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZORRO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.zorro/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zorro profile create ls' would be rejected.",
    "zorro profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zorro claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZORRO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zorro tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.zorro/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.zorro/skills/.hub/quarantine/ holds skills pending security review.",
]
