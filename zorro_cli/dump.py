"""
Dump command for zorro CLI.

Outputs a compact, plain-text summary of the user's Zorro setup
...
from zorro_cli.config import get_zorro_home, get_env_path, get_project_root, load_config
...
def _count_skills(zorro_home: Path) -> int:
...
def _cron_summary(zorro_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zorro_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zorro_home:      {display_zorro_home()}")
    ...
    toolsets = config.get("toolsets", ["zorro-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zorro_home)}")
    lines.append(f"  skills:             {_count_skills(zorro_home)}")
