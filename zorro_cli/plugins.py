...
1. **User plugins**   – ``~/.zorro/plugins/<name>/``
2. **Project plugins** – ``./.zorro/plugins/<name>/`` (opt-in via
   ``ZORRO_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``zorro_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "zorro_agent.plugins"

_NS_PARENT = "zorro_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``zorro honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.zorro/plugins/)
        user_dir = get_zorro_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.zorro/plugins/)
        if _env_enabled("ZORRO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".zorro" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``zorro_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``zorro tools`` TUI so plugin-provided toolsets appear
