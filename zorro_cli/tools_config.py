"""
Unified tool configuration for Zorro Agent.

`zorro tools` and `zorro setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.zorro/config.yaml under
...
from zorro_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ZORRO_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from zorro_cli.plugins import discover_plugins, get_plugin_toolsets
...
from zorro_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_zorro_home()}/zorro-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in zorro-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "zorro-cli" (which include all _ZORRO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "zorro-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `zorro tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by zorro tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (zorro-cli, zorro-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.zorro/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `zorro tools` and `zorro setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Zorro Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://zorro-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from zorro_constants import display_zorro_home
    print(color(f"  Tool configuration saved to {display_zorro_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'zorro' or gateway restart.", Colors.DIM))
    print()
