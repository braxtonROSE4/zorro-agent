"""
Status command for zorro CLI.

Shows the status of all Zorro Agent components.
"""
...
from zorro_cli.auth import AuthError, resolve_provider
from zorro_cli.colors import Colors, color
from zorro_cli.config import get_env_path, get_env_value, get_zorro_home, load_config
from zorro_cli.models import provider_label
from zorro_cli.nous_subscription import get_nous_subscription_features
from zorro_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Zorro Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Zorro Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from zorro_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: zorro model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: zorro model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zorro model)"
    ...
    if not terminal_env:
        ...
        # (zorro status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    platforms = {
        ...
        "Teams": ("TEAMS_APP_ID", "TEAMS_HOME_CHANNEL"),
    }
    ...
    if _is_termux():
        try:
            from zorro_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   zorro gateway")
        ...
        from zorro_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_zorro_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_zorro_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zorro doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zorro setup' to configure", Colors.DIM))
    print()
