"""
Configuration management for Zorro Agent.

Config files are stored in ~/.zorro/ for easy access:
- ~/.zorro/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zorro/.env         - API keys and secrets

This module provides:
- zorro config          - Show current configuration
- zorro config edit     - Open config in editor
- zorro config set      - Set a specific value
- zorro config wizard   - Re-run setup wizard
"""
...
from zorro_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZORRO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zorro_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zorro is running in package-manager-managed mode.

    Two signals: the ZORRO_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZORRO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade zorro-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "zorro update"


def format_managed_message(action: str = "modify this Zorro installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZORRO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zorro installation is managed by NixOS "
            f"(ZORRO_MANAGED={env_hint}).\n"
            "Edit services.zorro-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zorro installation is managed by Homebrew "
            f"(ZORRO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zorro-agent"
        )

    return (
        f"Cannot {action}: this Zorro installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zorro."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZORRO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zorro_bin
    or None if container mode is not active, we're already inside the
    container, or ZORRO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZORRO_DEV") == "1":
        return None
    ...
    container_mode_file = get_zorro_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zorro-agent")
    exec_user = info.get("exec_user", "zorro")
    zorro_bin = info.get("zorro_bin", "/data/current-package/bin/zorro")
...
# Re-export from zorro_constants — canonical definition lives there.
from zorro_constants import get_zorro_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zorro_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zorro_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zorro group can
    share state with the gateway service.

    The mode can be overridden via the ZORRO_HOME_MODE environment variable
    (e.g. ZORRO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZORRO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZORRO_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZORRO_HOME if the user doesn't have one yet."""
...
def ensure_zorro_home():
    """Ensure ~/.zorro directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_zorro_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_zorro_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZORRO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Zorro
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Zorro sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zorro/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.zorro/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for zorro-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Zorro Agent*" header.
    ...
    # Logging — controls file logging to ~/.zorro/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Zorro auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zorro:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZORRO_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zorro-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # ZORRO_TOOL_PROGRESS and ZORRO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zorro.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zorro setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Zorro won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zorro doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZORRO_TOOL_PROGRESS")
            old_mode = get_env_value("ZORRO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZORRO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZORRO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZORRO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZORRO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: zorro config set <key> <value>")
    ...
    # metadata.zorro.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zorro/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zorro/config.yaml."""
    import copy
    ensure_zorro_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — zorro auth) — OpenAI Codex
#   nous         (OAuth — zorro auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.zorro/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zorro/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zorro env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zorro/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zorro/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zorro/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zorro/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zorro — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zorro vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zorro/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Zorro Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  zorro config edit     # Edit config file", Colors.DIM))
    print(color("  zorro config set <key> <value>", Colors.DIM))
    print(color("  zorro setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zorro config set <key> <value>")
            print()
            print("Examples:")
            print("  zorro config set model anthropic/claude-sonnet-4")
            print("  zorro config set terminal.backend docker")
            print("  zorro config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zorro config migrate' to add them")
        ...
        print("  zorro config           Show current configuration")
        print("  zorro config edit      Open config in editor")
        print("  zorro config set <key> <value>   Set a config value")
        print("  zorro config check     Check for missing/outdated config")
        print("  zorro config migrate   Update config with new options")
        print("  zorro config path      Show config file path")
        print("  zorro config env-path  Show .env file path")
        sys.exit(1)
