"""
Single source of truth for provider identity in Zorro Agent.
...
2. **Zorro overlays** — transport type, auth patterns, aggregator flags,
...
# -- Zorro overlay ----------------------------------------------------------
# Zorro-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZorroOverlay:
    """Zorro-specific provider metadata layered on top of models.dev."""
...
ZORRO_OVERLAYS: Dict[str, ZorroOverlay] = {
    "openrouter": ZorroOverlay(
    ...
    "openai-codex": ZorroOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": ZorroOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZORRO_QWEN_BASE_URL",
    ),
    "copilot-acp": ZorroOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZorroOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZorroOverlay(
    ...
    "kimi-for-coding": ZorroOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": ZorroOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": ZorroOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ZorroOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ZorroOverlay(
        transport="openai_chat",
    ...
    "opencode": ZorroOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ZorroOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zorro", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Zorro overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Zorro overlay
    ...
    overlay = ZORRO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zorro extra
    ...
    if overlay is not None:
        # Zorro-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
