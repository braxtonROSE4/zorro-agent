...
Add, remove, or reorder entries here — both `zorro setup` and
`zorro` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `zorro model`
    flow without maintaining a separate static list.
    """
    from zorro_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from zorro_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``zorro model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``zorro auth add openai-codex`` without
    ``zorro model``).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from zorro_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Zorro's canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``zorro_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Zorro should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from zorro_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from zorro_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from zorro_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "ZorroAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from zorro_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Zorro will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
