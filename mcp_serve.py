"""
Zorro MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zorro-specific extra)

Usage:
    zorro mcp serve
    zorro mcp serve --verbose
...
logger = logging.getLogger("zorro.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZORRO_HOME."""
    try:
        from zorro_constants import get_zorro_home
        return get_zorro_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZORRO_HOME", Path.home() / ".zorro")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zorro_constants import get_zorro_home
        directory_file = get_zorro_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZORRO_HOME", Path.home() / ".zorro")
...
class EventBridge:
    ...
    This is the Zorro equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zorro_constants import get_zorro_home
            db_file = get_zorro_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZORRO_HOME", Path.home() / ".zorro")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zorro MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'zorro-agent[mcp]'"
        )

    mcp = FastMCP(
        "zorro",
        instructions=(
            "Zorro Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zorro MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
