...
from zorro_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZORRO_PLATFORM`` or
            ``ZORRO_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zorro/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zorro/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zorro = metadata.get("zorro") or {}
    if not isinstance(zorro, dict):
        zorro = {}
    return {
        "fallback_for_toolsets": zorro.get("fallback_for_toolsets", []),
        "requires_toolsets": zorro.get("requires_toolsets", []),
        "fallback_for_tools": zorro.get("fallback_for_tools", []),
        "requires_tools": zorro.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    zorro = metadata.get("zorro")
    if not isinstance(zorro, dict):
        return []
    raw = zorro.get("config")
    if not raw:
        return []
