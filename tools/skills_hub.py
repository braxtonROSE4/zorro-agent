#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Zorro Skills Hub.
...
Used by zorro_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zorro_meta = metadata.get("zorro", {})
            if isinstance(zorro_meta, dict):
                tags = zorro_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Brax) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.zorro/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                zorro_meta = meta_block.get("zorro", {})
                if isinstance(zorro_meta, dict):
                    tags = zorro_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Zorro centralized index source
# ---------------------------------------------------------------------------

ZORRO_INDEX_URL = "https://zorro-agent.nousresearch.com/docs/api/skills-index.json"
ZORRO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "zorro-index.json"
ZORRO_INDEX_TTL = 6 * 3600  # 6 hours


def _load_zorro_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZORRO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ZORRO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ZORRO_INDEX_CACHE_FILE.stat().st_mtime
            if age < ZORRO_INDEX_TTL:
                return json.loads(ZORRO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ZORRO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Zorro index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Zorro index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ZORRO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ZORRO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZorroIndexSource(SkillSource):
    """Skill source backed by the centralized Zorro Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zorro-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zorro-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZorroIndexSource(auth=auth), # Centralized index (search + resolved install paths)
