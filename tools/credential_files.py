...
def _resolve_zorro_home() -> Path:
    from zorro_constants import get_zorro_home
    return get_zorro_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.zorro",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ZORRO_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ZORRO_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    zorro_home = _resolve_zorro_home()

    # Reject absolute paths — they bypass the ZORRO_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ZORRO_HOME)",
            relative_path,
        )
        return False

    host_path = zorro_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ZORRO_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, zorro_home)
...
def _load_config_files() -> List[Dict[str, str]]:
    ...
    try:
        from zorro_cli.config import read_raw_config
        zorro_home = _resolve_zorro_home()
        cfg = read_raw_config()
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.zorro/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="zorro-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching zorro_constants.get_zorro_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_zorro_dir()`` for backward compatibility with old directory layouts.
    """
    from zorro_constants import get_zorro_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_zorro_dir(new_subpath, old_name)
        if host_dir.is_dir():
