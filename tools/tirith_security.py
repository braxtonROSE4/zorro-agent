...
it is automatically downloaded from GitHub releases to $ZORRO_HOME/bin/tirith.
...
def _get_zorro_home() -> str:
    """Return the Zorro home directory, respecting ZORRO_HOME env var."""
    return str(get_zorro_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zorro_home(), ".tirith-install-failed")
...
def _zorro_bin_dir() -> str:
    """Return $ZORRO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zorro_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZORRO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zorro_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZORRO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZORRO_HOME/bin/tirith
    ...
    zorro_bin = os.path.join(_zorro_bin_dir(), "tirith")
    if os.path.isfile(zorro_bin) and os.access(zorro_bin, os.X_OK):
        _resolved_path = zorro_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zorro_bin
