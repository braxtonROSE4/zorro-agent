...
This module provides the glue:
    - ``ZorroTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Zorro Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ZORRO_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZORRO_HOME/mcp-tokens/``
    """
    try:
        from zorro_constants import get_zorro_home
        base = Path(get_zorro_home())
    except ImportError:
        base = Path(os.environ.get("ZORRO_HOME", str(Path.home() / ".zorro")))
...
# ---------------------------------------------------------------------------
# ZorroTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZorroTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZORRO_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZORRO_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Zorro.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ZorroTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Zorro Agent")
    scope = cfg.get("scope")
