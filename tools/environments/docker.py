...
from tools.environments.local import _ZORRO_PROVIDER_ENV_BLOCKLIST
...
def _load_zorro_env_vars() -> dict[str, str]:
    """Load ~/.zorro/.env values without failing Docker command execution."""
    try:
        from zorro_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.zorro/sandboxes/). Non-persistent
        ...
        container_name = f"zorro-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Zorro secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ZORRO_PROVIDER_ENV_BLOCKLIST)
        zorro_env = _load_zorro_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = zorro_env.get(key)
