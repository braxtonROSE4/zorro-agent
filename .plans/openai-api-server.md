# OpenAI-Compatible API Server for Zorro Agent
...
SSE streaming. By exposing this endpoint, zorro-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  zorro-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zorro/.env`
2. Run `zorro gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zorro-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zorro-agent as a model) |
...
Authorization: Bearer zorro-api-key-here
...
But zorro-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zorro-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zorro-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zorro-agent's core prompt
...
Once implemented, zorro-agent works as a drop-in backend for:
