...
```bash
zorro memory setup    # select "holographic"
```

Or manually:
```bash
zorro config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.zorro-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZORRO_HOME/memory_store.db` | SQLite database path |
