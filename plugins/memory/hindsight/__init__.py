...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: zorro)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $ZORRO_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ZORRO_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_zorro_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "zorro": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "zorro"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Zorro Agent and the User"
    ...
    def save_config(self, values, zorro_home):
        """Write config to $ZORRO_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(zorro_home) / "hindsight"
    ...
    def post_setup(self, zorro_home: str, config: dict) -> None:
        ...
        from zorro_cli.memory_setup import _curses_select
        ...
        provider_config["bank_id"] = "zorro"
        provider_config["recall_budget"] = "mid"
        bank_id = "zorro"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, zorro_home)

        if env_writes:
            env_path = Path(zorro_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "zorro"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Zorro Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "zorro"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "zorro"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("zorro", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "zorro")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Zorro Agent and the User")
