...
```bash
zorro memory setup    # select "supermemory"
```

Or manually:

```bash
zorro config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zorro/.env
```

## Config

Config file: `$ZORRO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zorro` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zorro-{identity}` → `zorro-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Zorro profile:

```json
{
  "container_tag": "zorro-{identity}"
}
```

For a profile named `coder`, this resolves to `zorro-coder`. The default profile resolves to `zorro-default`. Without `{identity}`, all profiles share the same container.
