...
def _load_supermemory_config(zorro_home: str) -> dict:
    config = _default_config()
    config_path = Path(zorro_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zorro_home: str) -> None:
    config_path = Path(zorro_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zorro memory setup`.
        # All other options are documented for $ZORRO_HOME/supermemory.json
    ...
    def save_config(self, values, zorro_home):
        ...
        _save_supermemory_config(sanitized, zorro_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from zorro_constants import get_zorro_home
        self._zorro_home = kwargs.get("zorro_home") or str(get_zorro_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zorro_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "zorro", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "zorro_memory", "target": target, "type": "explicit_memory"},
