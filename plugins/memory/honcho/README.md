...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zorro>
...
```bash
zorro honcho setup    # full interactive wizard (cloud or local)
zorro memory setup    # generic picker, also works
```

Or manually:
```bash
zorro config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.zorro/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZORRO_HOME/honcho.json` | Profile-local (isolated Zorro instances) |
| 2 | `~/.zorro/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Zorro profile: `zorro` (default) or `zorro.<profile>`.
...
Host key derivation: `ZORRO_HONCHO_HOST` env > active profile (`zorro.<profile>`) > `"zorro"`.
...
| `ZORRO_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `zorro honcho setup` | Full interactive setup wizard |
| `zorro honcho status` | Show resolved config for active profile |
| `zorro honcho enable` / `disable` | Toggle Honcho for active profile |
| `zorro honcho mode <mode>` | Change recall or observation mode |
| `zorro honcho peer --user <name>` | Update user peer name |
| `zorro honcho peer --ai <name>` | Update AI peer name |
| `zorro honcho tokens --context <N>` | Set context token budget |
| `zorro honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zorro honcho map <name>` | Map current directory to a session name |
| `zorro honcho sync` | Create host blocks for all Zorro profiles |
