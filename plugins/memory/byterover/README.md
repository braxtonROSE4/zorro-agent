...
```bash
zorro memory setup    # select "byterover"
```

Or manually:
```bash
zorro config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.zorro/.env
```
...
Working directory: `$ZORRO_HOME/byterover/` (profile-scoped).
