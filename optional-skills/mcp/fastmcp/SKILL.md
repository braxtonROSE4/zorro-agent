...
- smoke-test a server with the FastMCP CLI before wiring it into Zorro or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zorro. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zorro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zorro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Zorro integration, either:

- configure the server in `~/.zorro/config.yaml` using the `native-mcp` skill, or
...
### Zorro cannot see the deployed server

The server-building part may be correct while the Zorro config is not. Load the `native-mcp` skill and configure the server in `~/.zorro/config.yaml`, then restart Zorro.
