...
version: 1.0.0
author: arceus77-7, enhanced by Zorro Agent
license: MIT
metadata:
  zorro:
...
- `tmux` available for stable authenticated sessions during Zorro terminal calls (desktop app flow only)
...
### Service Account (recommended for Zorro)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.zorro/.env` (the skill will prompt for this on first load).
...
## Zorro Execution Pattern (desktop app flow)

Zorro terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zorro-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zorro-op.sock"
