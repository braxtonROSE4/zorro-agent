...
This is an optional third-party workflow, not a Zorro core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Zorro native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Zorro native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Zorro native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Zorro usage patterns
...
- Do not silently choose Parallel when Zorro native tools are already sufficient.
