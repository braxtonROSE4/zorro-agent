#!/usr/bin/env python3
"""Google Workspace API CLI for Zorro Agent.

Thin wrapper that delegates to gws (googleworkspace/cli) via gws_bridge.py.
Maintains the same CLI interface for backward compatibility with Zorro skills.
...
def gws(*args: str) -> None:
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "ZORRO_HOME": os.environ.get("ZORRO_HOME", str(Path.home() / ".zorro"))},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zorro Agent (gws backend)")
