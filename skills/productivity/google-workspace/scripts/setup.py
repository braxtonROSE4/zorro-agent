#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Zorro Agent.
...
try:
    from zorro_constants import display_zorro_home, get_zorro_home
except ModuleNotFoundError:
    ZORRO_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if ZORRO_AGENT_ROOT.exists():
        sys.path.insert(0, str(ZORRO_AGENT_ROOT))
    from zorro_constants import display_zorro_home, get_zorro_home

ZORRO_HOME = get_zorro_home()
TOKEN_PATH = ZORRO_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZORRO_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZORRO_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Zorro profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Zorro home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_zorro_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Zorro")
