...
Restart Zorro Agent. On startup it will:
...
1. Reads `mcp_servers` from `~/.zorro/config.yaml`
2. For each server, spawns a connection in a dedicated background event loop
3. Initializes the MCP session and calls `list_tools()` to discover available tools
4. Registers each tool in the Zorro tool registry
...
For stdio servers, Zorro does NOT pass your full shell environment to MCP subprocesses. Only safe baseline variables are inherited:
...
No `mcp_servers` key in `~/.zorro/config.yaml`, or it's empty. Add at least one server.
...
- Look at Zorro Agent startup logs for connection messages
...
Zorro supports MCP's `sampling/createMessage` capability — MCP servers can request LLM completions through the agent during tool execution. This enables agent-in-the-loop workflows (data analysis, content generation, decision-making).
