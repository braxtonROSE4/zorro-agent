...
- Give it a name like "zorro-agent"
- Select scopes:
...
- Give it a title like "zorro-agent-<machine-name>"
...
elif [ -f ~/.zorro/.env ] && grep -q "^GITHUB_TOKEN=" ~/.zorro/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.zorro/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
