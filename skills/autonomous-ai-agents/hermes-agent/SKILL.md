...
zorro mcp configure NAME   Toggle tool selection
```

### Gateway (Messaging Platforms)

```
zorro gateway run          Start gateway foreground
...
zorro gateway setup        Configure platforms
```
...
zorro sessions stats       Session store statistics
```

### Cron Jobs

```
zorro cron list            List jobs (--all for disabled)
...
zorro cron status          Scheduler status
```

### Webhooks

```
zorro webhook subscribe N  Create route at /webhooks/<name>
zorro webhook list         List subscriptions
zorro webhook remove NAME  Remove a subscription
zorro webhook test NAME    Send a test POST
```

### Profiles

```
zorro profile list         List all profiles
...
zorro profile import FILE  Import from archive
```

### Credential Pools

```
zorro auth add             Interactive credential wizard
zorro auth list [PROVIDER] List pooled credentials
zorro auth remove P INDEX  Remove by provider + index
zorro auth reset PROVIDER  Clear exhaustion status
```

### Other

```
zorro insights [--days N]  Usage analytics
...
zorro uninstall            Uninstall Zorro
```
...
```
~/.zorro/config.yaml       Main configuration
...
~/.zorro/zorro-agent/     Source code (if git-installed)
```

Profiles use `~/.zorro/profiles/<name>/` with the same layout.
...
## Spawning Additional Zorro Instances
...
terminal(command="zorro chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### Interactive PTY Mode (via tmux)

Zorro uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
terminal(command="tmux new-session -d -s resumed 'zorro --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **Use `zorro chat -q` for fire-and-forget** — no PTY needed
...
### Tool not available
1. `zorro tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `zorro doctor` — check config and dependencies
2. `zorro login` — re-authenticate OAuth providers
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `zorro model` → GitHub Copilot.
...
### Skills not showing
1. `zorro skills list` — verify installed
2. `zorro skills config` — check platform enablement
3. Load explicitly: `/skill name` or `zorro -s name`

### Gateway issues
Check logs first:
```bash
grep -i "failed to send\|error" ~/.zorro/logs/gateway.log | tail -20
```

Common gateway problems:
- **Gateway dies on SSH logout**: Enable linger: `sudo loginctl enable-linger $USER`
- **Gateway dies on WSL2 close**: WSL2 requires `systemd=true` in `/etc/wsl.conf` for systemd services to work. Without it, gateway falls back to `nohup` (dies when session closes).
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed zorro-gateway`
...
zorro config set auxiliary.vision.model <model_name>
```
...
|----------------|----------|
| Config options | `zorro config edit` or [Configuration docs](https://zorro-agent.nousresearch.com/docs/user-guide/configuration) |
| Available tools | `zorro tools list` or [Tools reference](https://zorro-agent.nousresearch.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://zorro-agent.nousresearch.com/docs/reference/slash-commands) |
| Skills catalog | `zorro skills browse` or [Skills catalog](https://zorro-agent.nousresearch.com/docs/reference/skills-catalog) |
| Provider setup | `zorro model` or [Providers guide](https://zorro-agent.nousresearch.com/docs/integrations/providers) |
| Platform setup | `zorro gateway setup` or [Messaging docs](https://zorro-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP servers | `zorro mcp list` or [MCP guide](https://zorro-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `zorro profile list` or [Profiles docs](https://zorro-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron jobs | `zorro cron list` or [Cron docs](https://zorro-agent.nousresearch.com/docs/user-guide/features/cron) |
| Memory | `zorro memory status` or [Memory docs](https://zorro-agent.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `zorro config env-path` or [Env vars reference](https://zorro-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI commands | `zorro --help` or [CLI reference](https://zorro-agent.nousresearch.com/docs/reference/cli-commands) |
...
Config: `~/.zorro/config.yaml` (settings), `~/.zorro/.env` (API keys).
...
1. Add `CommandDef` to `COMMAND_REGISTRY` in `zorro_cli/commands.py`
...
- Tests auto-redirect `ZORRO_HOME` to temp dirs — never touch real `~/.zorro/`
...
- Use `get_zorro_home()` from `zorro_constants` for all paths (profile-safe)
