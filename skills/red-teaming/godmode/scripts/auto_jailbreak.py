...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.zorro/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZORRO_HOME", Path.home() / ".zorro")) / "skills" / "red-teaming" / "godmode"
...
ZORRO_HOME = Path(os.getenv("ZORRO_HOME", Path.home() / ".zorro"))
CONFIG_PATH = ZORRO_HOME / "config.yaml"
PREFILL_PATH = ZORRO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Zorro models — already uncensored, just needs clean prompt
    "zorro": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zorro" in model_lower or "nous" in model_lower:
        return "zorro"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zorro config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zorro/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zorro for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zorro for changes to take effect.")
