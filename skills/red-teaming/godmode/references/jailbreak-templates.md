...
## 5. GODMODE FAST — Zorro 4 405B (Zero Refusal)

**Model:** `nousresearch/zorro-4-405b`
**Strategy:** Zorro models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Zorro Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.zorro/config.yaml`:
