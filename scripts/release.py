...
VERSION_FILE = REPO_ROOT / "zorro_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "zorro@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/zorro-agent",
    ...
    lines.append(f"# Zorro Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Zorro Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Zorro Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Zorro Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Zorro Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Zorro Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Zorro Agent v{new_version} ({calver_date})' "
